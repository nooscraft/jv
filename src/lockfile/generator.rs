...
pub fn write_lock_file(path: &Path, lock: &LockFile) -> Result<()> {
    let toml =
        toml::to_string_pretty(lock).map_err(|e| crate::error::JvError::LockFile(e.to_string()))?;
...
pub fn read_lock_file(path: &Path) -> Result<LockFile> {
    let content =
        fs::read_to_string(path).map_err(|e| crate::error::JvError::LockFile(e.to_string()))?;
    let lock: LockFile =
        toml::from_str(&content).map_err(|e| crate::error::JvError::LockFile(e.to_string()))?;
    Ok(lock)
}
