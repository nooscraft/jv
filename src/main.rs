...
async fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Resolve(args) => {
            ...
            if file.extension().is_some_and(|e| e == "xml") || file.ends_with("pom.xml") {
                ...
                .is_some_and(|e| e == "gradle" || e == "kts")
            {
                ...
                println!(
                    "Unsupported build file. Supported: pom.xml, build.gradle, build.gradle.kts"
                );
            }
        }
        Commands::Verify => {
                    ...
                    match cache.prune(max_age_days) {
                        Ok((count, bytes)) => {
                            if count > 0 {
                                ...
                                println!(
                                    "Cache prune: nothing older than {} days to remove.",
                                    max_age_days
                                );
                            }
                        }
...
fn resolve_build_file(path: &Path) -> Option<PathBuf> {
    if path.is_file() {
