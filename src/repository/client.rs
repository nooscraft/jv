...
impl RepositoryClient {
    ...
    pub async fn fetch_metadata(&self, group_id: &str, artifact_id: &str) -> Result<MavenMetadata> {
        let path = format!("{}/{}/maven-metadata.xml", group_id.replace('.', "/"), artifact_id);
...
mod tests {
    ...
    async fn can_fetch_a_well_known_pom() {
        ...
        assert!(pom.is_ok(), "should successfully fetch commons-lang3 POM from Maven Central");
        let pom_xml = pom.unwrap();
        assert!(pom_xml.contains("commons-lang3"), "POM should contain the artifact name");
    }
}
