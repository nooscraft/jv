    ...
    async fn for_coordinate_impl(
        ...
        for pom in chain.iter().rev() {
            ...
            properties.insert(
                ...
                pom.coordinate.version.raw.clone(),
            );
            ...
            for dm in &pom.dependency_management {
                ...
                if dm.scope == Scope::Import {
                    ...
                    if let Some(cached) = cache.get_bom_effective(
                        ...
                        for (imp_key_str, ver_str) in cached.managed_versions {
                            if let Some((g, a)) = imp_key_str.split_once(':') {
                                ...
                                let resolved =
                                    crate::resolver::effective::interpolate(&properties, &ver_str);
                    ...
                    match Box::pin(Self::for_coordinate_impl(
                        bom_coord,
                        client,
                        cache,
                        no_cache,
                    ...
                    .await
                    {
                        Ok(imported_effective) => {
                            ...
                            for (imp_key, imp_dm) in &imported_effective.dependency_management {
                                let resolved_ver_str = crate::resolver::effective::interpolate(
                                    &properties,
                                    &imp_dm.coordinate.version.raw,
                                );
                            ...
                            for (imp_key, imp_dm) in &imported_effective.dependency_management {
                                let resolved = crate::resolver::effective::interpolate(
                                    &imported_effective.properties,
                                    &imp_dm.coordinate.version.raw,
                                );
                        ...
                        Err(e) => {
                            debug!(
                                "Failed to build effective view for imported BOM {}: {}",
                                bom_coord, e
                            );
